...
class Cliente {
    ...
    private static final int dadosFSChunk = 1000;
     ...
     * @param portaUDP Porta UDP que o Servidor tem aberta para receber o(s) pacote(s) UDP (80).
     * @param portaTCP Porta ACK usada para receber os pacotes ACK vindos do Servidor durante a transferência (9999).
     * @param localDisco A diretoria+nome do ficheiro a ser enviado.
     * @param enderecoIP Endereço IP do servidor a enviar o ficheiro.
     */
    Cliente(int portaUDP, int portaTCP, String localDisco, String enderecoIP) {
        ...
        try {
            ...
            socketRececao = new DatagramSocket(portaTCP);
                    ...
                    do {
                        ...
                        if(numeroSequenciaEnviar < numSeqBaseJanela + 5000) {
                            ...
                            if(numeroSequenciaEnviar < pacotesJanelaDeslizante.size()) {
                                ...
                                byte[] dataBuffer = new byte[dadosFSChunk];
                                int tamanhoDados = ficheiro.read(dataBuffer, 0, dadosFSChunk);

                                // Ainda existem dados para enviar.
                                if(tamanhoDados != -1) {
                                    byte[] dataBytes = Arrays.copyOfRange(dataBuffer, 0, tamanhoDados);
                                    dadosEnviados = FSChunk.gerarPacoteDados(numeroSequenciaEnviar, dataBytes);
                                ...
                                if(tamanhoDados == -1) {
                                    ultimoNumSeq = 0;
                                    dadosEnviados = FSChunk.gerarPacoteDados(numeroSequenciaEnviar, new byte[0]);
            ...
            try {
                ...
                try {
                    do {
                        ...
                        int numACK = FSChunk.getACK(recebeDados);
                        ...
                        if(numACK == -5) {
                            ...
                            numSeqBaseJanela = numACK + dadosFSChunk;
