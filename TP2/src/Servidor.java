...
class Servidor {
     ...
     * @param portaUDP Porta UDP usada para receber o(s) pacote(s) UDP (80).
     * @param localDisco A diretoria para onde o ficheiro a ser recebido irá ser colocado.
     */
    public Servidor(int portaUDP,int portaTCP, String localDisco) {
        ...
        try {
            ...
            try {
                ...
                do {
                    ...
                    // Cliente envia 0 para o Servidor saber que deve começar os seus pedidos.
                    ...
                    if(numSeqPacoteRecebido == proximo) {
                        ...
                        if(pacoteRecebido.getLength() == 4) {
                            ...
                            byte[] pacoteFinal = Reply.gerarPacoteReply(-5);
                            socketEnvio.send(new DatagramPacket(pacoteFinal, pacoteFinal.length, ipAddress, portaTCP));

                            System.out.println("Servidor >TCP final.");
                        ...
                        // Se pacote nem é o último nem o primeiro (intermediários).
                            ...
                            proximo = numSeqPacoteRecebido + 1000;

                            // Envia o ACK para avisar que recebeu aquele número de sequência.
                            byte[] pacote = Reply.gerarPacoteReply(proximo);
                            socketEnvio.send(new DatagramPacket(pacote, pacote.length, ipAddress, portaTCP));

                            StringBuilder FSChunk = new StringBuilder("Servidor > TCP ");
                            FSChunk.append(proximo);
                            System.out.println(FSChunk);
                        }
                    ...
                    // Assim o Cliente sabe que o Servidor em si não recebeu aquilo que pediu.
                        ...
                        byte[] pacote = Reply.gerarPacoteReply(numeroSequencia);
                        socketEnvio.send(new DatagramPacket(pacote, pacote.length, ipAddress, portaTCP));

                        StringBuilder pacoteDuplicado = new StringBuilder("Servidor >  duplicado ");
